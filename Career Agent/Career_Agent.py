...
tools_list = [search]
agent = initialize_agent(tools = tools_list, llm = model, verbose=True, agent = "zero-shot-react-description")
...
def search_jobs_tavily(query: str) -> str:
  """Search for jobs using Tavily and returns summarized results."""
...
def matcher_node(state: List[BaseMessage]) -> List[BaseMessage]:
  ...
  prompt = f"given this profile:\n{profile}\n\n Rank and recommend top3 jobs from:\n{jobs}"
